...
class LocationInputWidget extends StatefulWidget {
  ...
  final void Function(PlaceLocationModal location) onSelectedLocation;
...
class _LocationInputWidgetState extends State<LocationInputWidget> {
  ...
  Future<void> _savePlace(double latitude, double longitude) async {
    // to get more knowledge about it see the videos again
    final url = Uri.parse(
        'https://maps.googleapis.com/maps/api/geocode/json?latlng=$latitude,$longitude&key=AIzaSyBfszrvr6tTGmMfarhe-QHTtzctpeT2mFA');

    final response = await http
        .get(url); // as to create the body for json we have to use the await
    final resData = json.decode(response.body);
    // print(lat);
    // print(lng);
    final address = resData['results'][0][
        'formatted_address']; // result is the default key generated by the map api
    //print(address);
    setState(() {
      _pickedLocation = PlaceLocationModal(
        latitude: latitude,
        longitude: longitude,
        address: address,
      );
